...
- **Chamadas à API REST** encapsuladas em `web/src/db/api.js`, separando a camada de dados da camada de apresentação.
...
| `web/src/db/api.js` | Cliente Fetch no frontend com as mesmas assinaturas da camada de banco anterior. |
...
- Implementada via `LanguageContext` em `web/src/contexts/LanguageContext.jsx`.
...
├── package.json               # Scripts do workspace (frontend + testes)
├── vite.config.js             # Vite (root em web/) + proxy /api → :3001
│
...
│   │   ├── App.jsx
│   │   └── main.jsx
│   └── e2e/                   # Playwright fixtures, helpers, pages e specs
│
