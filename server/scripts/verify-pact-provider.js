...
async function startProviderIfNeeded() {
  try {
    ...
    console.log(`[PACT/server] Provider já estava disponível em ${providerBaseUrl}`);
    return;
  } catch {
    // iniciar localmente
  }
...
async function verifyProvider() {
  ...
  const verifierOptions = {
    ...
    providerVersion: process.env.PACT_PROVIDER_VERSION || process.env.GIT_COMMIT || 'local-dev-server',
...
verifyProvider()
  .then(() => {
    console.log('[PACT/server] Provider verification concluída com sucesso.');
  })
  .catch((error) => {
    console.error('[PACT/server] Falha na provider verification:', error?.message || error);
    process.exitCode = 1;
  })
  .finally(async () => {
